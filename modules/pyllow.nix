              ...
              ({
                ...
                config = lib.mkIf config.pyllow.enable (
                  let
                    ...
                    envExportString = lib.concatStringsSep "\n" (map envToBash (builtins.filter (env: !(lib.elem env.name ["PATH" "NIXPKGS_PATH"])) config.env));
                    ...
                    packagesWithoutTools = builtins.filter (pkg: !(lib.elem (pnameOf pkg) (map pnameOf matchedTools))) (config.packages or []);
                  ...
                  in {
                    name = lib.mkDefault "pyllow-devshell";
                    motd = ''
                      ...
                      ++ (
                        ...
                        then [
                          ...
                          }
                          {
                            ...
                            value =
                              lib.makeLibraryPath (config.packages
                                ++ allLibraries);
                          }
                        ]
                        else []
